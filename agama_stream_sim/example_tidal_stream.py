...
'''
import os, time as walltime, numpy, agama, scipy.special, scipy.integrate, matplotlib.pyplot as plt
try: import pyfalcon
...
def orbitDF(ic, time, timestart, trajsize, mass):
    ...
    traj = scipy.integrate.odeint(
        lambda xv, t: numpy.hstack((xv[3:6], pot_host.force(xv[0:3], t=t) + dynfricAccel(xv[0:3], xv[3:6], mass) )),
        ic, times)
    return times, traj
...
if plot:
    plt.ion()
    # Interactive mode by itself does not make every Matplotlib backend display
    # the window immediately.  Explicitly show and service the GUI event loop so
    # each freshly-computed simulation interval is visible as it completes.
...
while time < tend:
    step_started = walltime.perf_counter()
    ...
    time += tupd
    print('%5.3f  %.4g' % (time, r_mass[-1]/initmass) +
        ('  %.4g' % (f_mass[-1]/initmass) if pyfalcon else '') +
        '  [live step %.2fs]' % (walltime.perf_counter() - step_started), flush=True)
    ...
    if plot or time==tend:
        ...
        bx1.plot(times_t, numpy.sum(numpy.vstack(r_traj)[:,0:3]**2, axis=1)**0.5, 'r')
        cx1.plot(times_u, r_mass, 'b')
        if pyfalcon:
            times_f = numpy.arange(len(f_traj)) * tau
            bx2.plot(times_f, numpy.sum(numpy.vstack(f_traj)[:,0:3]**2, axis=1)**0.5, 'r')
            cx2.plot(times_f, f_mass, 'b')
        bx1.set_ylim(0, R0)
        bx2.set_ylim(0, R0)
        ...
        fig.canvas.draw_idle()
        fig.canvas.flush_events()
        # Pace fast frames at 60 Hz. If the physics takes longer than one frame
        # period, render immediately instead of adding more delay.
